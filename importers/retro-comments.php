...
<div class="wrap">
	<h1><?php esc_html_e( 'Retroactive Comments', 'mycred_retro' ); ?> <a href="<?php echo esc_url(admin_url( 'import.php' )); ?>" class="page-title-action"><?php esc_html_e( 'All Tools', 'mycred_retro' ); ?></a></h1>
	<div id="message" class="info notice notice-info"><p><?php esc_html_e( 'Remember to disable this plugin once you are done using it!', 'mycred_retro' ); ?></p></div>
<?php

			if ( $total == 0 ) {

?>
	<div id="message" class="updated notice"><p><?php esc_html_e( 'No comments found.', 'mycred_retro' ); ?></p></div>
<?php
...
?>
	<form id="import-upload-form" method="post" action="">
		<table class="wp-list-table widefat fixed striped users">
			<thead>
				<tr>
					<th style="width: 30%;"><?php esc_html_e( 'Comment Status', 'mycred_retro' ); ?></th>
					<th style="width: 20%;"><?php esc_html_e( 'Point Type', 'mycred_retro' ); ?></th>
					<th style="width: 15%;"><?php esc_html_e( 'Amount', 'mycred_retro' ); ?></th>
					<th style="width: 25%;"><?php esc_html_e( 'Log Entry Template', 'mycred_retro' ); ?></th>
					<th style="width: 10%;"><?php esc_html_e( 'Comments', 'mycred_retro' ); ?></th>
				</tr>
			</thead>
			<tbody>
				<tr>
					<td>
						<select name="mycred_retro[comment_status]" id="select-comment-status" class="form-control">
							<option value="" data-template="" data-count="<?php echo esc_attr($total); ?>" data-all="<?php echo esc_attr($total_all); ?>"><?php esc_html_e( 'Select Comment Status', 'mycred_retro' ); ?></option>
							<option value="approved" data-template="%plural% for approved comment"<?php if ( $approved == 0 ) echo ' disabled="disabled"'; ?> data-count="<?php echo esc_attr($approved); ?>" data-all="<?php echo esc_attr($approved_all); ?>"><?php esc_html_e( 'Approved Comments', 'mycred_retro' ); ?></option>
							<option value="trash" data-template="%plural% for deleted comment"<?php if ( $trashed == 0 ) echo ' disabled="disabled"'; ?> data-count="<?php echo esc_attr($trashed); ?>" data-all="<?php echo esc_attr($trashed_all); ?>"><?php esc_html_e( 'Trashed Comments', 'mycred_retro' ); ?></option>
							<option value="spam" data-template="%plural% for SPAM comment"<?php if ( $spam == 0 ) echo ' disabled="disabled"'; ?> data-count="<?php echo esc_attr($spam); ?>" data-all="<?php echo esc_attr($spam_all); ?>"><?php esc_html_e( 'Marked as SPAM Comments', 'mycred_retro' ); ?></option>
						</select>
					</td>
					<td>
					...
					<td>
						<input type="text" name="mycred_retro[amount]" placeholder="<?php esc_html_e( 'amount', 'mycred_retro' ); ?>" id="comment-log-amount" class="form-control" value="" />
					</td>
					<td>
						<input type="text" name="mycred_retro[log_template]" id="comment-log-template" placeholder="<?php esc_html_e( 'no log entry', 'mycred_retro' ); ?>" class="form-control" value="" />
					</td>
					<td id="comment-count"><h5><?php echo esc_html($total); ?></h5></td>
				</tr>
			</tbody>
		</table>
		<p>
			<label for="comment-original-date"><input type="checkbox" name="mycred_retro[date]" id="comment-original-date" checked="checked" value="1" /> <?php esc_html_e( 'When adding a log entry, use the date the comment was originally added, and not the current date.', 'mycred_retro' ); ?></label>
			<label for="comment-original-visitors"><input type="checkbox" name="mycred_retro[visitor]" id="comment-original-visitors" value="1" /> <?php esc_html_e( 'Some comments were left by users while they were logged out. If the comment authors email matches a user on the website, credit that user for the comment.', 'mycred_retro' ); ?></label>
		</p>
		...
		<div id="task-status" style="display: none;">
			...
			<div id="stop-tool-action">
				<button type="button" id="cancel-task" class="button button-secondary button-large"><?php esc_html_e( 'Stop Task', 'mycred_retro' ); ?></button>
			</div>
			<h3><?php esc_html_e( 'Task Report', 'mycred_retro' ); ?></h3>
			<table class="wp-list-table widefat fixed striped users" id="task-progress-table">
				<thead>
					<tr>
						<th style="width: 20%;"><?php esc_html_e( 'Eligible Comments', 'mycred_retro' ); ?></th>
						<th style="width: 20%;"><?php esc_html_e( 'Completed', 'mycred_retro' ); ?></th>
						<th style="width: 20%;"><?php esc_html_e( 'Excluded', 'mycred_retro' ); ?></th>
						<th style="width: 20%;"><?php esc_html_e( 'Missing User', 'mycred_retro' ); ?></th>
						<th style="width: 20%;"><?php esc_html_e( 'Remaining', 'mycred_retro' ); ?></th>
					</tr>
				</thead>
				<tbody>
					<tr>
		...
		$( '#comment-original-visitors' ).click(function(){
			...
			if ( $(this).is( ':checked' ) ) {
				...
				if ( selectedoption !== undefined ) {
					...
					$( '#comment-count h5' ).empty().text( "<?php echo esc_html($total_all); ?>" );
					$( '#tak-report-total' ).empty().text( "<?php echo esc_html($total_all); ?>" );
				}
				...
				if ( selectedoption !== undefined ) {
					...
					$( '#comment-count h5' ).empty().text( "<?php echo esc_html($total); ?>" );
					$( '#tak-report-total' ).empty().text( "<?php echo esc_html($total); ?>" );
				}
...
<?php
		...
		static function ajax_handler() {
			...
			$post_task  = isset($_POST['task']) ? absint($_POST['task']) : 0;

			$post_offset  = isset($_POST['offset']) ? absint($_POST['offset']) : 0;
			...
			$args = shortcode_atts( array(
				...
				'original_date' => 'true',
				'visitors'      => 'false'
			), array_map( 'sanitize_text_field',$post_task  ) );
